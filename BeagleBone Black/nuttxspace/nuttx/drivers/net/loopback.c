...
static void lo_txavail_work(FAR void *arg)
{
  ...
  ninfo("lo_txavail_work: Called for device %s (ifindex: %d)\n", priv->lo_dev.d_ifname, priv->lo_dev.d_ifindex);
  ...
  if (priv->lo_bifup)
    {
      /* First, check for packet socket listeners */

#ifdef CONFIG_NET_PKT
      ninfo("lo_txavail_work: Calling pkt_input for ifindex: %d\n", priv->lo_dev.d_ifindex);
