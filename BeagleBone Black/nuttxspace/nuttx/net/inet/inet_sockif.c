      ...
      case SOCK_RAW:
        {
          ...
          dev = netdev_findby_lipv4addr(htonl(INADDR_LOOPBACK));
          if (dev == NULL)
            {
              nerr("ERROR: No device found for loopback\n");
              return -ENODEV;
            }

          ninfo("Loopback device found: %s, ifindex: %d\n", dev->d_ifname, dev->d_ifindex);
          conn->ifindex = dev->d_ifindex;
          ((FAR struct socket_conn_s *)psock->s_conn)->s_flags |= _SF_BOUND;
          ret = OK;
        }
        break;
#endif
  ...
  switch (psock->s_type)
    ...
    case SOCK_DGRAM:
      ...
      case SOCK_RAW:
      {
        ...
        /* DIAGNOSTIC: Print pointers to confirm identity */
        ninfo("DEBUG: recvmsg psock=%p, conn=%p, ifindex_before=%d\n", psock, conn, conn->ifindex);

        /* CRITICAL HARDWARE PATCH: Force Loopback Binding
         * If the ifindex has been lost (reset to 0), force it back to 1 (lo).
         * This ensures the Packet Driver (pkt_input) can match the incoming
         * loopback frames to this connection.
         */
        if (conn->ifindex == 0)
          {
             nwarn("WARNING: Restoring lost ifindex to 1 (Loopback)\n");
             conn->ifindex = 1;
          }

        /* 1. Delegate receive to the packet layer */
        ret = pkt_recvmsg(psock, msg, flags);

        /* 2. Post-processing: Filter and Strip Headers */
        if (ret > 0)
          {
             /* We assume Ethernet headers (14 bytes) */
             const size_t eth_hdr_len = 14; 

             if (ret > eth_hdr_len)
               {
                 FAR uint8_t *buf = (FAR uint8_t *)msg->msg_iov->iov_base;
                 FAR struct eth_hdr_s *eth = (FAR struct eth_hdr_s *)buf;

                 /* MANUAL FILTER: Check EtherType for IPv4 (0x0800)
                  * Since the driver delivers everything, we must drop ARPs/IPv6 here.
                  */
                 if (eth->type != HTONS(ETHTYPE_IP))
                   {
                      /* Not IPv4 - Drop this packet and tell caller to try again.
                       * Returning 0 or EAGAIN depends on blocking semantics, but 
                       * usually 0 (or continuing a loop) is best. 
                       * For now, return 0 to indicate "Message read but empty/ignored".
                       */
                      ninfo("Ignored non-IPv4 packet type: %04x\n", ntohs(eth->type));
                      ret = 0;
                   }

                 /* Strip Ethernet Header: Move payload to front */
                 memmove(buf, buf + eth_hdr_len, ret - eth_hdr_len);
                 ret -= eth_hdr_len;
               }
             else
               {
