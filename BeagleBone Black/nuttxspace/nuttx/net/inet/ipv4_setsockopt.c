                    ...
                    FAR const void *value, socklen_t value_len)
{
  ...
  switch (option)
    {
      case IP_HDRINCL:
        /* For our NuttX SOCK_RAW implementation (which delegates to packet sockets),
         * the kernel does not generate IP headers anyway. The user provides them.
         * Therefore, we treat this option as supported and successfully set.
         * We perform no action other than returning OK to satisfy the application.
         */
        ninfo("ipv4_setsockopt: IP_HDRINCL set (implicit for SOCK_RAW)\n");
        ret = OK;
        break;
