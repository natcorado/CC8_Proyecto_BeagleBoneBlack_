                    ...
                    int flags)
{
  ...
  /* CRITICAL: Check if this is the Loopback Device */
  /* We can check d_ifindex == 1 or d_lltype == NET_LL_LOOPBACK (if defined) */
  if (dev->d_ifindex == 1) 
  {
      /* 1. Construct a Fake Ethernet Header */
      /* pkt_input expects [Dest MAC | Src MAC | EtherType | Payload] */
      struct eth_hdr_s
      {
         uint8_t  dest[6];
         uint8_t  src[6];
      ...
      /* Zero out MACs (Loopback doesn't care, but pkt_input offsets need them) */
      memset(fake_eth.dest, 0, 6);
      memset(fake_eth.src, 0, 6);
      
      /* Set EtherType to IPv4 (0x0800) so your filter accepts it */
      fake_eth.type = HTONS(ETHTYPE_IP); // Ensure <nuttx/net/ethernet.h> is included

      /* 2. Copy Header to Device Buffer */
      /* Check bounds! CONFIG_NET_ETH_PKTSIZE or similar should be checked */
      memcpy(dev->d_buf, &fake_eth, 14);

      /* 3. Copy User Payload (IP Header + Data) after Ethernet Header */
      memcpy(dev->d_buf + 14, buf, len);

      /* 4. Set Total Length */
      dev->d_len = len + 14;

      /* 5. DIRECT INJECTION: Feed it back into the input */
      /* This simulates the "interrupt" of a packet arriving */
      pkt_input(dev);

      /* 6. Clean up */
      dev->d_len = 0; /* Mark buffer as free */
      
      net_unlock();
      return len; /* Return success to the application */
  }
